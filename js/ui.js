...
// =================================================================
// 🏫 1. ฟังก์ชัน Render Navbar ธีมเลือดหมู-ทอง พร้อมตราโรงเรียน
// =================================================================
// =================================================================
// 🏫 1. ฟังก์ชัน Render Navbar ฉบับอัปเกรดความพรีเมียม (ดีไซน์หรูหรามีมิติ)
// =================================================================
// =================================================================
// 🏫 1. ฟังก์ชัน Render Navbar ฉบับสมบูรณ์พรีเมียม (ซ่อมสัดส่วนชื่อ ยศ และปุ่มประตู)
// =================================================================
// =================================================================
// 🏫 1. ฟังก์ชัน Render Navbar (เวอร์ชันแยกรักษายศ กรอบสีรุ้งแอนิเมชันสำหรับ SUPERADMIN)
// =================================================================
    ...
    navContainer.innerHTML = `
        <nav class="bg-[#7A1C1C] text-white shadow-2xl border-b-4 border-[#D4AF37] p-4 select-none">
            <div class="container mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="flex items-center space-x-3.5 cursor-pointer bg-black/15 border border-amber-400/20 px-4 py-2.5 rounded-2xl shadow-inner backdrop-blur-sm group hover:border-amber-400/40 transition duration-300" id="brandLink">
                    <div class="w-14 h-14 bg-white rounded-full border-2 border-[#D4AF37] shadow-md overflow-hidden flex items-center justify-center shrink-0 group-hover:scale-105 transition duration-300">
                        <img src="assets/logo/BS.jfif" alt="ตราโรงเรียนเบญจมราชานุสรณ์" class="w-full h-full object-cover scale-105" onerror="this.src='https://placehold.co/100x100/7A1C1C/FFFFFF?text=BS'">
                    </div>
                    <div class="flex flex-col">
                        <span class="text-xl font-black tracking-widest text-white drop-shadow-[0_1.5px_1.5px_rgba(0,0,0,0.6)]">B.S. PORTFOLIO HUB</span>
                ...
                <div class="flex items-center flex-wrap justify-center gap-4 text-xs font-bold">
                    ...
                    <button id="homeLink" class="relative group px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-gray-100 hover:text-amber-300 transition duration-200 cursor-pointer overflow-hidden shadow-sm border border-white/5">
                        <span class="relative z-10 flex items-center gap-1">🏡 หน้าแรก</span>
...
export function renderPortfolios(portfolios, filters = { keyword: "", university: "ทั้งหมด", faculty: "ทั้งหมด", round: "ทั้งหมด" }) {
    ...
    // คัดเฉพาะพอร์ตที่ผ่านการอนุมัติ (approved) หรือยังไม่มีสถานะ เพื่อนำมาคำนวณยอดสถิติหน้าบ้าน
    const approvedList = portfolios.filter(p => p.status === 'approved' || !p.status);
    const totalCount = approvedList.length;

    // --- 🧮 ตรรกะประมวลผลสรุปสถิติหาอันดับ 1 (TOP 1) แบบเรียลไทม์ ---
    const uniCounts = {};
    ...
    // --- 🏛️ กลุ่มรายชื่อสถาบันและคณะยอดนิยมระดับประเทศสำหรับตัวเลือก Dropdown ---
    ...
    // พ่นโครงสร้าง HTML หน้าแรกหลักแบบพรีเมียมลงบน Container
    appContainer.innerHTML = `
        <div class="my-6">
            <div class="border-b-2 border-[#7A1C1C] pb-3 mb-6">
                <h1 class="text-3xl font-bold text-[#7A1C1C]">คลังผลงานรุ่นพี่</h1>
                <p class="text-gray-600 text-sm mt-1">แหล่งรวมข้อมูล สถิติ และแรงบันดาลใจในการทำพอร์ตโฟลิโอจากพี่สู่น้องโรงเรียนเบญจมราชานุสรณ์</p>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl border-2 border-[#7A1C1C]/20 shadow-lg mb-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    <div class="bg-gray-50/60 p-3 rounded-xl border border-gray-200 flex flex-col justify-between">
                        ...
                        <input type="text" id="search-keyword" value="${filters.keyword}" placeholder="พิมพ์คำค้นหา..." 
                               class="w-full p-2.5 text-xs font-bold text-gray-800 border-2 border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#7A1C1C] focus:border-[#7A1C1C] bg-white transition duration-200 shadow-sm">
                    </div>
                    ...
                    <div class="bg-gray-50/60 p-3 rounded-xl border border-gray-200 flex flex-col justify-between">
                        ...
                        <select id="search-university" class="w-full p-2.5 text-xs font-bold text-gray-800 border-2 border-gray-300 rounded-lg bg-white shadow-sm cursor-pointer focus:ring-2 focus:ring-[#7A1C1C] outline-none">
                            ...
                            <optgroup label="📝 สถาบันอื่น ๆ ที่ตรวจเจอ">
                                ${otherUniversities.map(uni => `<option value="${uni}" ${filters.university === uni ? 'selected' : ''}>${uni}</option>`).join('')}
                            </optgroup>
                            ` : ''}
                        </select>
                    </div>
                    ...
                    <div class="bg-gray-50/60 p-3 rounded-xl border border-gray-200 flex flex-col justify-between">
                        ...
                        <select id="search-faculty" class="w-full p-2.5 text-xs font-bold text-gray-800 border-2 border-gray-300 rounded-lg bg-white shadow-sm cursor-pointer focus:ring-2 focus:ring-[#7A1C1C] outline-none">
                            ...
                            <optgroup label="📝 คณะอื่น ๆ ที่ตรวจเจอ">
                                ${otherFaculties.map(fac => `<option value="${fac}" ${filters.faculty === fac ? 'selected' : ''}>${fac}</option>`).join('')}
                            </optgroup>
                            ` : ''}
                        </select>
                    </div>
                ...
                <div class="mt-5 pt-3 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3 text-xs">
                    ...
                    <button id="clear-filters-btn" class="text-xs text-[#7A1C1C] hover:text-red-700 font-black bg-amber-50/60 border border-[#D4AF37]/40 px-4 py-2 rounded-xl shadow-sm cursor-pointer">
                        🧹 ล้างค่าค้นหาและตัวกรองทั้งหมด
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-8 border-t border-gray-200 mb-12">
                <div class="bg-white p-6 rounded-2xl shadow-md border border-gray-200">
                    <h3 class="text-base font-bold text-[#7A1C1C] mb-4 flex items-center gap-2">
                        <span>🏛️</span> 10 อันดับสถาบันยอดนิยมสูงสุด
                    </h3>
                    <div class="space-y-3">
                            ...
                            return `
                                <div class="space-y-1">
                                    <div class="flex justify-between text-xs font-semibold text-gray-700">
                                        <span>อันดับ ${idx + 1}: ${uni[0]}</span>
                                        <span class="text-[#7A1C1C] font-bold">${uni[1]} พอร์ต</span>
                                    </div>
                                    <div class="w-full bg-gray-100 rounded-full h-2">
                                        <div class="bg-[#D4AF37] h-2 rounded-full" style="width: ${percent}%"></div>
                                    </div>
                                </div>
                            `;
                        }).join('')}
                ...
                <div class="bg-white p-6 rounded-2xl shadow-md border border-gray-200">
                    <h3 class="text-base font-bold text-[#7A1C1C] mb-4 flex items-center gap-2">
                        <span>🎓</span> 10 อันดับคณะที่มีสถิติสอบติดสูงสุด
                    </h3>
                    <div class="space-y-3">
                        ${top10Faculties.length === 0 ? '<p class="text-xs text-gray-400 text-center py-4">ยังไม่มีข้อมูลในระบบ</p>' : top10Faculties.map((fac, idx) => {
                            ...
                            return `
                                <div class="space-y-1">
                                    <div class="flex justify-between text-xs font-semibold text-gray-700">
                                        <span>อันดับ ${idx + 1}: ${fac[0]}</span>
                                        <span class="text-[#7A1C1C] font-bold">${fac[1]} พอร์ต</span>
                                    </div>
                                    <div class="w-full bg-gray-100 rounded-full h-2">
                                        <div class="bg-[#7A1C1C] h-2 rounded-full" style="width: ${percent}%"></div>
                                    </div>
                                </div>
                            `;
                        }).join('')}
            ...
            <footer class="mt-16 bg-[#7A1C1C] text-white rounded-3xl border-b-4 border-[#D4AF37] p-6 shadow-xl select-none">
                <div class="flex flex-col sm:flex-row items-center justify-center gap-5">
                    ...
                    <div class="w-16 h-16 bg-white rounded-full border-2 border-[#D4AF37] shadow-md overflow-hidden flex items-center justify-center shrink-0 shadow-inner">
                        <img src="assets/logo/CM.jpg" alt="ตราแผนการเรียน Technology Software & Robotics" class="w-full h-full object-cover scale-110" onerror="this.src='https://placehold.co/100x100/7A1C1C/FFFFFF?text=CM'">
                    </div>
                    ...
                    <div class="text-center sm:text-left flex flex-col justify-center">
                        <span class="text-base font-black tracking-widest bg-gradient-to-r from-amber-200 via-yellow-300 to-amber-100 bg-clip-text text-transparent drop-shadow-[0_1px_1.5px_rgba(0,0,0,0.5)]">
                            TECHNOLOGY SOFTWARE & ROBOTICS
                        </span>
                        <span class="text-[11px] font-bold text-amber-200/90 tracking-wide mt-0.5">
                            แผนการเรียนเทคโนโลยีซอฟต์แวร์และหุ่นยนต์ • โรงเรียนเบญจมราชานุสรณ์
                        </span>
                        <p class="text-[9px] font-medium text-red-200/60 mt-1">
                            © 2026 B.S. Portfolio Hub. All Rights Reserved. จัดทำขึ้นเพื่อสนับสนุนการศึกษาและแนะแนวน้อง ๆ บ.ส.
                        </p>
                    </div>
    ...
    // สั่งเติมข้อมูลการ์ดผลงานพอร์ตลงตะกร้าด้านในทันทีในการโหลดครั้งแรก
