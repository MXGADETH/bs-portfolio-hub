...
export function updatePortfolioListUI(portfolios, filters) {
    ...
    const sortSelect = document.getElementById('search-sort'); // ตัวเลือกเรียงลำดับ
    if (!cardsContainer) return;

    const safetyPortfolios = Array.isArray(portfolios) ? portfolios : [];

    // 1. กรองข้อมูล (Filter) - โค้ดเดิมของนาย
    ...
    // 2. เรียงลำดับ (Sort) - 🎯 ใส่เพิ่มให้ในฟังก์ชันนี้
    ...
    // 3. วาดการ์ด (ใส่ Icon ลูกตาเข้าไปด้วย)
    cardsContainer.innerHTML = `
                ...
                <div class="bg-white rounded-2xl shadow-md hover:shadow-2xl border border-gray-200 overflow-hidden transition-all duration-300 transform hover:-translate-y-1.5 flex flex-col justify-between">
                    <div>
                        <div class="relative h-60 bg-gray-200 border-b overflow-hidden">
                            ...
                            <span class="absolute top-3 right-3 bg-[#7A1C1C] text-white text-xs px-2.5 py-1 rounded-full font-black shadow">${p.round || 'ไม่ระบุรอบ'}</span>
                        </div>
                    ...
                    <div class="p-5 pt-0 mt-auto space-y-2">
                        <button class="view-details-btn block w-full text-center bg-[#7A1C1C] text-white font-black py-2.5 px-4 rounded-xl text-xs border-b-4 border-[#D4AF37] shadow-sm cursor-pointer" data-fullname="${p.fullname}" data-title="${p.title}">🔍 ดูรายละเอียด</button>
...
export function renderPortfolioModal(portfolio) {
    ...
    modalOverlay.innerHTML = `
        <div id="modalContainer" class="bg-white rounded-3xl max-w-4xl w-full max-h-[90vh] shadow-2xl overflow-hidden transform scale-95 transition-transform duration-300 flex flex-col border border-gray-100">
...
export function renderPortfolioForm(containerId, username) {
    ...
    container.innerHTML = `
        <div class="max-w-3xl mx-auto my-6 bg-white p-8 rounded-2xl border border-gray-200 shadow-xl">
            ...
            <form id="portfolio-form" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-xs font-black text-gray-700 mb-1">🎯 รอบการคัดเลือก</label>
    ...
    // ระบบ Event Delegation: ดักฟังการเปลี่ยนค่าใน container ตัวแม่
