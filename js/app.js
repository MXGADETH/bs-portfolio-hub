...
function initFilterEvents() {
    ...
    // ⚡ ตัวประมวลผลหลัก: ทำงานทันทีที่มีการพิมพ์หรือคลิกตัวเลือก (เวอร์ชันลื่นไหล ดึงจาก Cache)
    const handleFiltering = () => {
       const currentFilters = getScreenFilters();
    
    // 🔒 เปลี่ยนจาก state.portfolios มาใช้ข้อมูลในถังพัก cachedPortfolios แทน (ถ้าไม่มีให้เคลียร์เป็นอาเรย์ว่าง [])
        const dataToRender = cachedPortfolios || [];
    
    // สั่งล้างและวาดใหม่เฉพาะการ์ดพอร์ตโฟลิโอ
        updatePortfolioListUI(dataToRender, currentFilters);
    };
...
// 1. สร้างถังพักข้อมูลตัวแปรส่วนกลางสแตนด์บายไว้ในหน่วยความจำ (Global State)
let cachedPortfolios = null;

// 2. ฟังก์ชันดึงข้อมูลฉบับรีดความลื่น (โหลดแค่ครั้งเดียว)
async function getPortfoliosData() {
    // 🔥 ทริคเด็ด: ถ้าในถังพักมีข้อมูลเก่าอยู่แล้ว ส่งข้อมูลเก่าออกไปใช้งานทันที ไม่ต้องโหลดใหม่!
    if (cachedPortfolios !== null && cachedPortfolios.length > 0) {
        console.log("⚡ ดึงข้อมูลจาก Cache ในเครื่องมาใช้ทันที ลื่นปรื๊ด!");
        return cachedPortfolios;
    }

    try {
        // กรณีเปิดเว็บครั้งแรกสุด ถังพักยังว่างอยู่ ค่อยวิ่งไปโหลดจริงจากเซิร์ฟเวอร์
        console.log("🌐 โหลดข้อมูลสดๆ จากเซิร์ฟเวอร์ (รันเฉพาะรอบแรก)...");
        const response = await fetch('ลิงก์_API_Google_Apps_Script_ของนาย');
        const data = await response.json();
        
        // 🔒 นำข้อมูลที่โหลดได้รอบแรกเก็บล็อกไว้ในถังพักอย่างปลอดภัย
...
// 3. เวลาตัวกรอง 4 มิติทำงาน หรือเวลาเปลี่ยนหน้า
async function handleFilterOrPageChange() {
    // เรียกฟังก์ชันฉลาดตัวบน มันจะสับท่อดึงข้อมูลในเครื่องให้เองโดยไม่ยิง Fetch ใหม่
    const portfolios = await getPortfoliosData(); 
    
    // สั่งวาดเฉพาะการ์ดผลงานพอร์ตโฟลิโอออกหน้าจอ
    updatePortfolioListUI(portfolios, currentFilters);
}
